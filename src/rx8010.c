...
#define RX8010_I2C_BUS		"i2c1"		/* i2c linked */
#define	RX8010_DEVICE_NAME	"rtc"		/* register device name */

static struct rt_device rx8010_dev;		/* rx8010 device */
...
static rt_err_t  rx8010_read_reg(rt_device_t dev, rt_uint8_t reg,rt_uint8_t *data,rt_uint8_t data_size)
{
    ...
    msg[0].addr  = RX8010_ARRD;
    msg[0].flags = RT_I2C_WR;
    msg[0].len   = 1;
    msg[0].buf   = &reg;
    msg[1].addr  = RX8010_ARRD;
...
static rt_err_t  rx8010_write_reg(rt_device_t dev, rt_uint8_t reg, rt_uint8_t *data, rt_uint8_t data_size)
{
    struct rt_i2c_msg msg[2];
	struct rt_i2c_bus_device *i2c_bus = RT_NULL;
...
static rt_size_t rt_rx8010_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    return RT_EOK;
}
...
static rt_err_t rt_rx8010_control(rt_device_t dev, int cmd, void *args)
{
    ...
    switch (cmd)
    {
        ...
        case RT_DEVICE_CTRL_RTC_GET_TIME:
	        time = (time_t *)args;
	        ret = rx8010_read_reg(dev, REG_SEC, buff, 7);
			if(ret == RT_EOK)
			{
        ...
        case RT_DEVICE_CTRL_RTC_SET_TIME:
        {
            ...
            ret = rx8010_write_reg(dev, REG_SEC, buff, 7);
        }
        break;
		...
		{ 	
			...
			rt_uint8_t state[3] = {0x00};		
		  	...
		  	ret = rx8010_read_reg(dev, REG_ALM_MIN, buff, 2);
			if(ret != RT_EOK)
			{
				return -RT_ERROR;
			}
			...
			ret = rx8010_read_reg(dev, RGE_EXT, state, 3);
			if(ret != RT_EOK)
			{
				return -RT_ERROR;
			}
		...
		case RT_DEVICE_CTRL_RTC_SET_ALARM:
		{
			...
			ret = rx8010_read_reg(dev, REG_FLAG, ctrl, 2);
			if (ret != RT_EOK)
			{
				return -RT_ERROR;
			}
			...
			/* write to hardware */
			ret = rx8010_write_reg(dev, REG_ALM_MIN, buff, 3);
			if (ret != RT_EOK)
			{
				return -RT_ERROR;
			}
			...
			ret = rx8010_write_reg(dev, REG_FLAG, &ctrl[1], 1);
		}
		break;
	#endif
        default:
 ...
 *  @return 'RT_EOK' when  rx8010 init completed successfully, 
...
{	
    ...
    i2c_bus = rt_i2c_bus_device_find(RX8010_I2C_BUS);
    if (i2c_bus == RT_NULL)
    {
    ...
    /* register rtc device */
    rx8010_dev.type   	= RT_Device_Class_RTC;
    rx8010_dev.init    	= RT_NULL;
    rx8010_dev.open    	= rt_rx8010_open;
    rx8010_dev.close   	= RT_NULL;
    rx8010_dev.read   	= rt_rx8010_read;
    rx8010_dev.write  	= RT_NULL;
    rx8010_dev.control 	= rt_rx8010_control;
    rx8010_dev.user_data= (void*)i2c_bus;	/* save i2cbus */
    rt_device_register(&rx8010_dev, RX8010_DEVICE_NAME, RT_DEVICE_FLAG_RDWR);
		
    /* init rx8010 */
	if (rx8010_read_reg(&rx8010_dev, REG_FLAG, &flags, 1))
	{
		return -RT_ERROR;
	}
...
//		if (rx8010_write_reg(&rx8010_dev, REG_CTRL, &flags, 1) != RT_EOK)	/* clear ctrl register */
//		{
//			return -RT_ERROR;
//		}
	
		flags = 0x00;
		if (rx8010_write_reg(&rx8010_dev, RGE_EXT, &flags, 1) != RT_EOK)	/* set second update */
		{
			return -RT_ERROR;
		}
	}
	...
	{
		flags = 0x00;
		if (rx8010_write_reg(&rx8010_dev, REG_FLAG, &flags, 1) != RT_EOK)	/* clear flag register */
		{
			return -RT_ERROR;
		}	
	}
	
	LOG_D("the rtc of rx8010 init succeed!\r\n");
